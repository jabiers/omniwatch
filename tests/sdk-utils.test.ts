...
import { createSDK } from '../src/agent/sdk.js';
...
describe('SDK Utils', () => {
  ...
  it('retry should retry on failure then succeed', async () => {
    const sdk = createSDK();
    let attempts = 0;
    const result = await sdk.retry(async () => {
      attempts++;
  ...
  it('retry should throw after max retries', async () => {
    const sdk = createSDK();
    await expect(sdk.retry(async () => {
      throw new Error('always fails');
    }, { maxRetries: 2, delay: 10 })).rejects.toThrow('always fails');
  });
