...
export function enqueueMessage(topic: string, payload: unknown, fromAgent: string): number {
  ...
  const pending = (
    db
      ...
      .get(fromAgent) as { c: number }
  ).c;

  if (pending >= QUEUE_BACKPRESSURE_LIMIT) {
    log('warn', `Queue backpressure: agent ${fromAgent} has ${pending} pending messages`);
    throw new Error(
      `Queue backpressure: ${pending} pending messages exceeds limit ${QUEUE_BACKPRESSURE_LIMIT}`,
    );
  }

  const payloadStr = typeof payload === 'string' ? payload : JSON.stringify(payload);
  const result = db
    .prepare('INSERT INTO message_queue (topic, payload, from_agent) VALUES (?, ?, ?)')
...
export function nackMessage(id: number, errorMsg?: string): void {
  const db = getDb();
  const msg = db
    .prepare('SELECT id, topic, payload, retry_count FROM message_queue WHERE id = ?')
    .get(id) as Pick<QueueMessage, 'id' | 'topic' | 'payload' | 'retry_count'> | null;
  if (!msg) return;
  ...
  if (msg.retry_count >= QUEUE_MAX_RETRIES) {
    ...
    db.prepare("UPDATE message_queue SET status = 'failed' WHERE id = ?").run(id);
...
export function getQueueStats(): QueueStats {
  const db = getDb();
  const pending = (
    db.prepare("SELECT COUNT(*) as c FROM message_queue WHERE status = 'pending'").get() as {
      c: number;
    }
  ).c;
  const processing = (
    db.prepare("SELECT COUNT(*) as c FROM message_queue WHERE status = 'processing'").get() as {
      c: number;
    }
  ).c;
  const doneToday = (
  ...
  const deadLetters = (db.prepare('SELECT COUNT(*) as c FROM dead_letters').get() as { c: number })
    .c;
...
export function retryDeadLetter(id: number): boolean {
  const db = getDb();
  const dl = db.prepare('SELECT id, topic, payload FROM dead_letters WHERE id = ?').get(id) as Pick<
    DeadLetter,
    'id' | 'topic' | 'payload'
  > | null;
  if (!dl) return false;
