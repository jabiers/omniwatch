...
|   +-- daemon/                 # 엔진 모듈 (v2.0부터 API에 내장)
|   |   +-- src/engine.ts       # 엔진 라이프사이클 (initEngine/shutdownEngine)
|   |   +-- src/handlers/       # 8개 핸들러 그룹 (직접 함수 호출)
