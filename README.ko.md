...
OmniWatch는 원하는 작업을 자연어로 설명하면 AI가 자동으로 백그라운드 에이전트를 생성, 실행, 모니터링하고 24시간 자가 복구하는 플랫폼입니다. CLI, 백그라운드 데몬, REST API, 그리고 완전한 기능의 웹 대시보드를 제공합니다.
...
# 전체 시작 (API + Dashboard + Daemon watch 모드)
pnpm dev
...
- **마이그레이션**: 5개 버전별 마이그레이션 파일 (v001-v005)
...
|   +-- daemon/                 # 백그라운드 데몬
|   |   +-- src/handlers/       # 7개 RPC 핸들러 그룹
|   |   +-- src/agent/          # 에이전트 런타임 + SDK
|   |   +-- src/sandbox.ts      # VM 격리 + isolated-vm
