# OmniWatch - System Definition

## 1. System Identity

OmniWatch는 CLI 기반 AI 네이티브 자율형 에이전트 관리 플랫폼이다.
...
```
$ omni watch "쿠팡에서 에어팟 프로 25만원 이하면 알려줘"
...
- **CLI (`omni`)**: 경량 클라이언트. 명령 전달 후 종료. 터미널을 닫아도 에이전트는 유지.
- **Daemon (`omnid`)**: 백그라운드 상주. 에이전트 수명주기, 헬스체크, 알림, AI 호출 담당.
...
2. **Agent SDK**: 에이전트가 사용하는 표준 API (`omni.fetch`, `omni.notify`, `omni.store`)
...
1. **Command Mode**: `omni watch/list/logs/stop/start` 등 원샷 명령
2. **Interactive Mode**: `omni chat` 대화형 에이전트 생성/관리
3. **Dashboard Mode**: `omni dash` 실시간 TUI 대시보드 (Ink 기반)
4. **Log Streaming**: `omni logs <id> --follow` 실시간 로그 스트리밍

### E. The Foundation (인프라)

1. **SQLite**: 단일 파일 DB. 에이전트 메타, 로그, 알림 이력, KV 저장소
2. **Unix Domain Socket**: CLI ↔ Daemon IPC (JSON-RPC 2.0)
3. **Config**: `~/.omniwatch/config.toml` 전역 설정
...
| 유형 | 명령어 | 설명 | 예시 |
|------|--------|------|------|
| **Watcher** | `omni watch` | 조건 감시 → 충족 시 알림 | 가격, 주가, 웹 변경, 뉴스 |
| **Doer** | `omni do` | 주기적/단발 작업 실행 | 리포트 생성, 데이터 수집, 백업 |
| **Auto** | `omni auto` | 자율 판단 에이전트 | 이상 탐지, 원인 분석, 자동 대응 |
...
- **v0.1**: `npm i -g omniwatch` (npm global package)
