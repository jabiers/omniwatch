# Vigil v0.4 Plan - Monorepo + Web Dashboard

## 1. Overview

**Feature Name**: vigil-v0.4
**Version**: 0.4.0
...
- 플러그인 로딩: `~/.vigil/plugins/` 디렉토리에서 동적 import
- 플러그인 매니페스트: `plugin.json` (name, version, type, entry)
- 내장 채널(webhook, slack 등)을 플러그인으로 재구조화

### FR-06: npm Publish & Distribution
- `packages/vigil` (CLI+Daemon 통합) npm 배포 패키지
- `npm i -g vigil` 로 글로벌 설치
- `bin` 필드에 `vigil`, `vigild` 등록
...
- [ ] 기존 114 테스트 전수 통과
- [ ] `vigil watch/do/auto` CLI 기능 정상 동작
