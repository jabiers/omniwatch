# OmniWatch MVP Planning Document
...
OmniWatch는 터미널에서 자연어 한 줄로 모니터링/자동화 에이전트를 생성하고,
...
- [x] CLI 명령어 체계 (`omni watch/list/logs/stop/start/dash`)
- [x] 백그라운드 데몬 (`omnid`) + Unix Socket IPC
...
|----|-------------|----------|--------|
| **A. CLI** | | | |
| FR-01 | `omni watch "<prompt>"` - 자연어로 에이전트 생성 및 실행 | Critical | Pending |
| FR-02 | `omni list` - 에이전트 목록 조회 (상태, 마지막 체크 시간) | Critical | Pending |
| FR-03 | `omni logs <id> [--follow]` - 에이전트 로그 조회/스트리밍 | High | Pending |
| FR-04 | `omni stop/start/restart <id>` - 에이전트 제어 | Critical | Pending |
| FR-05 | `omni status <id>` - 에이전트 상세 상태 | High | Pending |
| FR-06 | `omni destroy <id>` - 에이전트 삭제 | High | Pending |
| FR-07 | `omni dash` - TUI 실시간 대시보드 | Medium | Pending |
| FR-08 | `omni config set/get` - 설정 관리 | High | Pending |
| **B. Daemon** | | | |
...
| FR-21 | 설정 파일 관리 (~/.omniwatch/config.toml) | High | Pending |
...
11. [ ] agent/sdk - omni.fetch, omni.notify, omni.store, omni.log
...
- [ ] `omni watch "..."` 로 에이전트가 생성되고 백그라운드에서 실행됨
- [ ] 터미널을 닫아도 에이전트가 계속 동작함
- [ ] `omni list` 로 실행 중인 에이전트 확인 가능
- [ ] 에이전트 오류 시 Self-Healing으로 자동 복구 (최소 1회)
- [ ] 조건 충족 시 Webhook 알림 발송
- [ ] `omni dash` 로 TUI 대시보드 확인 가능
